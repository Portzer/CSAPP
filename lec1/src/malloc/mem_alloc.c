...
uint64_t get_next_header(uint64_t vaddr)
{
    ...
    assert(get_first_block() < next_header_vaddr &&
           next_header_vaddr <= get_last_block());
...
uint64_t get_prev_header(uint64_t vaddr)
{
    ...
    uint32_t prev_blocksize = get_block_size(prev_footer_vaddr);

    uint64_t prev_header_vaddr = header_vaddr - prev_blocksize;
    assert(get_first_block() <= prev_header_vaddr &&
           prev_header_vaddr <= get_last_block());
...
uint64_t get_last_block()
{
    ...
    uint32_t last_blocksize = get_block_size(last_footer);
...
int is_last_block(uint64_t vaddr)
{
    ...
    uint64_t header_vaddr = get_header(vaddr);
    uint32_t blocksize = get_block_size(header_vaddr);
...
uint64_t get_field32_block_ptr(uint64_t header_vaddr, uint32_t min_blocksize, uint32_t offset)
{
    ...
    assert(get_block_size(header_vaddr) >= min_blocksize);
...
void set_field32_block_ptr(uint64_t header_vaddr, uint64_t block_ptr, uint32_t min_blocksize, uint32_t offset)
{
    ...
    assert(get_block_size(block_ptr) >= min_blocksize);
...
uint64_t merge_blocks_as_free(uint64_t low, uint64_t high)
{
    ...
    // must merge as free
    uint32_t blocksize = get_block_size(low) + get_block_size(high);

    set_block_size(low, blocksize);
    set_allocated(low, FREE);

    uint64_t footer = get_tail(high);
    set_block_size(footer, blocksize);
...
uint64_t try_alloc_with_splitting(uint64_t block_vaddr, uint32_t request_blocksize, uint32_t min_blocksize)
{
    ...
    uint32_t b_blocksize = get_block_size(b);
    ...
    if (b_allocated == FREE && b_blocksize >= request_blocksize)
    {
        ...
        if (b_blocksize - request_blocksize >= min_blocksize)
        {
            ...
            uint64_t next_footer = get_tail(b);
            set_allocated(next_footer, FREE);
            set_block_size(next_footer, b_blocksize - request_blocksize);

            set_allocated(b, ALLOCATED);
            set_block_size(b, request_blocksize);

            uint64_t b_footer = get_tail(b);
            set_allocated(b_footer, ALLOCATED);
            set_block_size(b_footer, request_blocksize);
            ...
            set_block_size(next_header, b_blocksize - request_blocksize);

            assert(get_tail(next_header) == next_footer);
...
uint64_t try_extend_heap_to_alloc(uint32_t size, uint32_t min_blocksize)
{
    ...
    uint32_t last_blocksize = get_block_size(old_last);
    ...
    if (os_allocated_size != 0)
    {
        ...
        if (last_allocated == ALLOCATED)
        {
            ...
            // set the new footer
            uint64_t new_last_footer = get_tail(new_last);
        ...
        else
        {
            ...
            set_block_size(old_last, last_blocksize + os_allocated_size);

            uint64_t last_footer = get_tail(old_last);
            set_allocated(last_footer, FREE);
            set_block_size(last_footer, last_blocksize + os_allocated_size);
